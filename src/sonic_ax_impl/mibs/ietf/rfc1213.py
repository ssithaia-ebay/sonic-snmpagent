...
class NetmaskUpdater(MIBUpdater):
    ...
    def update_data(self):
        ...
        mgmt_ip = os.popen('ip addr show eth0 | grep "\<inet\>" | awk \'{ print $2 }\'').read().strip()
        if (len(mgmt_ip) != 0):
            self._update_netmask_info("eth0", mgmt_ip)

        docker_inet = os.popen('ip addr show docker0 | grep "\<inet\>" | awk \'{ print $2 }\'').read().strip()
        if (len(docker_inet) != 0):
            self._update_netmask_info("docker0", docker_inet)

        docker_brd = os.popen('ip addr show docker0 | grep "\<inet\>" | awk \'{ print $4 }\'').read().strip()
        if (len(docker_brd) != 0):
            docker_brd = docker_brd + '/' + docker_inet.split('/')[1]
